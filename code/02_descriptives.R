...
hk_main_varlist <- c(
    "ln_houseprice", "kaufpreis", "price_sqmeter", "in_bau", "alter",
    "wohnflaeche", "grundstuecksflaeche",
    "badezimmer", "heizungsart", "ausstattung", "zimmeranzahl", "anzahletagen",
    "keller", "objektzustand", 
    "distance_station", "distance_junction", "distance_largcenter",
    "distance_medcenter", "distance_smalcenter",
    "distance_airport", "distance_industry", "distance_streets",
    "year_mon", "ajahr", "bf50_only", "bf100", "bf250", "bf500", "bf750",
    "bf1000", "bf500_only", "law_implementation"
)
...
hk_main_var_control <- subset(
    hk_main_var, hk_main_var$bf50_only != 1 &
    hk_main_var$bf100 != 1 &
    hk_main_var$bf250 != 1 &
    hk_main_var$bf500 != 1 &
    hk_main_var$bf750 != 1 &
...
# merge both tables
des_hk_control_treat <- merge(
    des_hk_bf500only_time,
    des_hk_bf500only_control_time,
    by = "variable"
)
...
# reorder column
des_hk_control_treat_wide$variable <- factor(
    des_hk_control_treat_wide$variable, 
    levels = c(
        "ln_houseprice", "kaufpreis", "zimmeranzahl", "alter", "anzahletagen",
        "ausstattung", "badezimmer", "grundstuecksflaeche", "heizungsart",
        "in_bau", "wohnflaeche", "objektzustand", "distance_largcenter",
        "distance_medcenter", "distance_smalcenter", "distance_airport",
        "distance_industry", "distance_streets", "distance_junction",
        "distance_station"
    )
)
...
des_hk_control_treat_wide <- des_hk_control_treat_wide |> 
    select(
        variable, mean_treat_control, mean_control_control,
...
reg_uncond_did <- function(data, depvar){
    ...
    fm <- formula(
        paste(
            depvar, "~",
            paste("bf500_only * law_implementation")
        )
    )

    # run regression
    est_mod <- feols(fm, data = regdata, se = "hetero")
...
num_obs <- hk_main_var |>
    group_by(bf500_only, law_implementation) |>
    summarise(N = n()) |> as.data.frame()

num_obs <- num_obs |>
    pivot_wider(values_from = N, names_from = law_implementation) |>
    as.data.frame()

colnames(num_obs) <- c("group", "control", "adoption", "actual_treat")

num_obs <- num_obs |> 
    tidyr::pivot_wider(values_from = c("control", "adoption", "actual_treat"), names_from = group) |>
    as.data.frame()

num_obs <- num_obs |> select(
    control_1, control_0,
    adoption_1, adoption_0,
    actual_treat_1, actual_treat_0
)
num_obs <- num_obs |> mutate(
    uncond_did_adoption_N = control_1 + control_0 + adoption_1 + adoption_0,
...
sum_price_month <- function(df, variable, name){
    sum_df <- df %>% group_by(year_mon) %>% summarise(
        mean_price_sqmeter = mean({{variable}}, na.rm = TRUE),
...
sum_periods_month <- function(df, variable, name){
    # average by period
    avg_control_period <- df %>%
        dplyr::filter(year_mon <= "2017-06") %>%
        dplyr::summarise(mean_diff = mean({{variable}}))

    avg_law_passed <- df %>%
        dplyr::filter(year_mon >= "2017-07" & year_mon <= "2020-11") %>%
        dplyr::summarise(mean_diff = mean({{variable}}))
    
    avg_law_in_force <- df %>%
        dplyr::filter(year_mon >= "2020-12") %>%
        dplyr::summarise(mean_diff = mean({{variable}}))
    
    # combine
    avg_differences_month <- data.frame(
        cbind(
            c("2013 Jan - 2017 Jun", "2017 Jul - 2020 Nov", "2020 Dec - 2021 Jun"),
...
##### merge
avg_differences <- cbind(
    avg_differences_bf50only, avg_differences_bf100[2], avg_differences_bf250[2], avg_differences_bf500[2],
...
sum_buffer_month_price <- sum_buffer_month_price %>% mutate(
    law_indicator = case_when(
        year_mon < "2017-07" ~ 0,
...
# grouping
sum_buffer_month <- sum_buffer_month %>% 
    mutate(
        cutoff = case_when(
            year_mon >= "2013-06" & year_mon <= "2017-06" ~ "control",
...
plot1 <- ggplot(data = sum_buffer_month, mapping = aes(x = plot_date, group = 1))+
    ...
    scale_linetype_manual(
        values = c(
            "mean_price_sqmeter_bf500only_control" = "solid",
            "mean_price_sqmeter_bf500only" = "twodash"
        ),
        labels = c(
            "mean_price_sqmeter_bf500only_control" = "beyond 500m (control)",
            "mean_price_sqmeter_bf500only" = "0-to-500m buffer (treated)"
        ),
        name = ""
    )+
    scale_x_yearmon(
        breaks = seq(min(sum_buffer_month$plot_date), max(sum_buffer_month$plot_date), 0.5),
        labels = month_labels
    )+
    scale_y_continuous(
        breaks = seq(1600, 3400, 200),
        labels = scales::comma
    )+
...
ggsave(
    ...
    file.path(
        outputPath,
        "graphs/plot_price_sqmeter_500only_control.png"
    ),
    width = 7.5,
    height = 5,
    dpi = 800
...
plot1 <- ggplot(data = sum_buffer_month, mapping = aes(x = plot_date, group = 1))+
    ...
    scale_linetype_manual(
        values = c(
            "mean_price_sqmeter_bf50only" = "twodash",
            "mean_price_sqmeter_bf100" = "dotted",
            "mean_price_sqmeter_control" = "solid"
        ),
        labels = c(
            "mean_price_sqmeter_bf50only" = "0-to-50m buffer",
            "mean_price_sqmeter_bf100" = "50-to-100m buffer",
            "mean_price_sqmeter_control" = "beyond 1000m (control)"
        ),
        name = "", 
        breaks = c(
            "mean_price_sqmeter_bf50only",
            "mean_price_sqmeter_bf100",
            "mean_price_sqmeter_control"
        )
    )+
    scale_x_yearmon(
        breaks = seq(min(sum_buffer_month$plot_date), max(sum_buffer_month$plot_date), 0.5),
...
ggsave(
    ...
    file.path(
        outputPath,
        "graphs/plot_price_sqmeter_50_100_control.png"
    ),
    width = 7,
    height = 5
)
...
plot1 <- ggplot(data = sum_buffer_month, mapping = aes(x = plot_date, group = 1))+
    ...
    scale_linetype_manual(
        values = c(
            "mean_price_sqmeter_bf250" = "dashed",
            "mean_price_sqmeter_bf500" = "dotted",
            "mean_price_sqmeter_control" = "solid"
        ),
        labels = c(
            "mean_price_sqmeter_bf250" = "100-to-250m buffer",
