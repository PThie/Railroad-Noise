...
readosm <- function(area){
  ...
  st_read(file.path(dataPath, paste0("motorway_junction/", area, ".osm")), 
          layer = "points") %>% 
    mutate(state = area) %>% 
...
lapply(names(query.state.part1.ls), function(x){
  ...
  osmdata_xml(f, paste0(dataPath, "motorway_junction/", x, ".osm"))})
...
saveRDS(motor.states.df, file.path(dataPath, "motorway_junction/ger_motorway_junctions.rds"))
