...
# grid data
microm <- haven::read_dta("M:/_FDZ/RWI-GEO/RWI-GEO-GRID/daten/Original/Stata16/V11/microm_panel_05-19.dta")
...
int_indep_ext <- c(
	"bf50_only * law_inprogress", "bf50_only * law_established",
...
# export
esttex(
	extended_hk, file = file.path(outputPath, "regression/hetero_extended_hk.tex"), replace = TRUE, digits = "r3", dict = tablabel_objchar,
...
##### plotting
extended_plot <- ggplot(
	data = coefs, 
    mapping = aes(
		x = factor(buffer, levels = c("50", "100", "250", "500", "750", "1000")),
		y = coef,
		col = factor(group, levels = c("inp", "est"))
	)
)+
	geom_point(
	mapping = aes(
	...
	scale_color_manual(
		values = c("inp" = "darkorange2", "est" = "royalblue4"),
...
ggsave(
	...
	file.path(
		outputPath,
		"graphs/extended_model_plot.png"
	)
)
...
extended_plot_german <- ggplot(
	...
	geom_pointrange(
		mapping = aes(x = buffer, ymin = (coef - (conf * se))*100, ymax = (coef + (conf * se))*100),
...
ggsave(
	plot = extended_plot_german,
	file.path(
		outputPath,
		"graphs/extended_model_plot_german.png"
	)
)
...
# export
esttex(
	baseline_region1, baseline_region2, baseline_region3, baseline_region4, file = file.path(outputPath, "regression/hetero_basemodel_hk_regtypes.tex"), replace = TRUE, digits = "r3", dict = tablabel_objchar,
...
microm_prep <- microm_prep %>% mutate(
	purchasing_power_cat = case_when(
		diff_pp >= quant_pp[1] & diff_pp < quant_pp[2] ~ 1,
...
# export
esttex(
	pp1_basemodel_hk, pp2_basemodel_hk, pp3_basemodel_hk, pp4_basemodel_hk,
	file = file.path(outputPath, "regression/hetero_basemodel_hk_richpoor.tex"),
	replace = TRUE, digits = "r3", dict = tablabel_objchar,
	signif.code = c("***" = 0.01, "**" = 0.05, "*" = 0.10),
	title = "heterogeneity basemodel hk rich vs poor",
...
# export
esttex(
	est_pp_below_med, est_pp_above_med,
	file = file.path(outputPath, "regression/hetero_basemodel_hk_richpoor_median.tex"),
	replace = TRUE,
	digits = "r3",
	dict = tablabel_objchar,
...
hk_affected <- hk_affected %>%
	mutate(
		settden_cat = case_when(
			settlement_density >= breaks_settden[1] & settlement_density < breaks_settden[2] ~ 1,
...
# export
esttex(
	settden1_basemodel_hk, settden2_basemodel_hk, settden3_basemodel_hk, settden4_basemodel_hk, file = file.path(outputPath, "regression/hetero_basemodel_hk_settlement_density.tex"), replace = TRUE, digits = "r3", dict = tablabel_objchar,
    signif.code = c("***" = 0.01, "**" = 0.05, "*" = 0.10),
	title = "heterogeneity basemodel hk settlement density",
...
# export
esttex(
	est_settden_below_med, est_settden_above_med,
	file = file.path(outputPath, "regression/hetero_basemodel_hk_settlement_density_median.tex"), replace = TRUE, digits = "r3", dict = tablabel_objchar,
    signif.code = c("***" = 0.01, "**" = 0.05, "*" = 0.10),
	title = "heterogeneity basemodel hk settlement density", 
...
# distances
dist_extreme <- as.data.frame(
	cbind(
		source = c("airport", "industry", "streets"), 
		first_quart = c(
			as.numeric(quantile(hk_affected$distance_airport, probs = 0.25, na.rm = TRUE)),
		...
		second_quart = c(
			as.numeric(quantile(hk_affected$distance_airport, probs = 0.5, na.rm = TRUE)),
...
extreme_quarter <- hk_affected %>%
	filter(
		distance_airport < as.numeric(quantile(distance_airport, probs = 0.25, na.rm = TRUE)) &
...
##### export
esttex(
	extreme_est_quarter, extreme_est_median, extreme_est_top, 
	file = file.path(outputPath, "regression/hetero_basemodel_hk_extreme_noises.tex"),
	replace = TRUE, digits = "r3", dict = tablabel_objchar,
	signif.code = c("***" = 0.01, "**" = 0.05, "*" = 0.10),
	title = "heterogeneity basemodel hk extreme noises", 
...
##### prepare
# extract data
coef_table_extreme <- rbind(
	as.data.frame(cbind(extreme_est_quarter$coeftable[39:40, 1:2], vars = c("law_passed", "law_inforce"), group = "first_quart")),
...
# confidence interval
conf <- 1.65
coef_table_extreme <- coef_table_extreme %>%
	mutate(
		conf_min = coef - (se * conf),
        conf_max = coef + (se * conf)
	)

##### plot
noises_extreme_plot <- ggplot(
	data = coef_table_extreme, 
    mapping = aes(
		x = factor(group, levels = c("first_quart", "second_quart", "top_quart")),
		y = coef,
		col = factor(vars, levels = c("law_passed", "law_inforce"))
	)
)+
...
ggsave(
	plot = noises_extreme_plot,
	file.path(outputPath, "graphs/noise_extreme.png"),
	dpi = 800
)
...
####### combined results #######
etable(
	dist_airport_belmed_est, dist_airport_abomed_est, 
...
esttex(
	...
	dist_industry_belmed_est, dist_industry_abomed_est, 
	dist_streets_belmed_est, dist_streets_abomed_est, 
	file = file.path(outputPath, "regression/hetero_basemodel_hk_noises_belabov_median.tex"),
	replace = TRUE, digits = "r3", dict = tablabel_objchar,
	signif.code = c("***" = 0.01, "**" = 0.05, "*" = 0.10),
	title = "heterogeneity basemodel noises belabov median", 
...
##### subset
dist_airport_high <- hk_affected %>%
	filter(
		distance_airport < as.numeric(quantile(distance_airport, probs = 0.25, na.rm = TRUE))
	)

dist_airport_medium <- hk_affected %>%
	filter(
		distance_airport >= as.numeric(quantile(distance_airport, probs = 0.25, na.rm = TRUE)) &
        distance_airport <= median(distance_airport, na.rm = TRUE)
	)

dist_airport_low <- hk_affected %>%
	filter(distance_airport > median(distance_airport, na.rm = TRUE))
...
##### export
esttex(
	dist_airport_high_est, dist_airport_medium_est, dist_airport_low_est,
	file = file.path(outputPath, "regression/hetero_basemodel_hk_airport_dist.tex"),
	replace = TRUE, digits = "r3", dict = tablabel_objchar,
	signif.code = c("***" = 0.01, "**" = 0.05, "*" = 0.10), 
	title = "heterogeneity basemodel airport distance", 
...
##### subset
dist_industry_high <- hk_affected %>%
	filter(
		distance_industry < as.numeric(quantile(distance_industry, probs = 0.25, na.rm = TRUE))
	)

dist_industry_medium <- hk_affected %>%
	filter(
		distance_industry >= as.numeric(quantile(distance_industry, probs = 0.25, na.rm = TRUE)) &
        distance_industry <= median(distance_industry, na.rm = TRUE)
	)

dist_industry_low <- hk_affected %>%
	filter(distance_industry > median(distance_industry, na.rm = TRUE))
...
##### export
esttex(
	dist_industry_high_est, dist_industry_medium_est, dist_industry_low_est,
...
##### subset
dist_streets_high <- hk_affected %>%
	filter(
		distance_streets < as.numeric(quantile(distance_streets, probs = 0.25, na.rm = TRUE))
	)

dist_streets_medium <- hk_affected %>%
	filter(
		distance_streets >= as.numeric(quantile(distance_streets, probs = 0.25, na.rm = TRUE)) &
        distance_streets <= median(distance_streets, na.rm = TRUE)
	)

dist_streets_low <- hk_affected %>%
	filter(distance_streets > median(distance_streets, na.rm = TRUE))
...
##### export
esttex(
	dist_streets_high_est, dist_streets_medium_est, dist_streets_low_est,
	file = file.path(outputPath, "regression/hetero_basemodel_hk_street_dist.tex"),
	replace = TRUE, digits = "r3", dict = tablabel_objchar,
	signif.code = c("***" = 0.01, "**" = 0.05, "*" = 0.10),
	title = "heterogeneity basemodel streets distance", 
	headers = c("high_exp", "med_exp", "low_exp")
)
...
noises_dist_plot <- ggplot(
	...
	facet_wrap(
		~ factor(group, levels = c("high", "medium", "low")),
		scales = "fixed",
...
ggsave(
	plot = noises_dist_plot,
	file.path(outputPath, "graphs/noise_distances.png"),
	width = 8,
	height = 7,
	dpi = 800
)
