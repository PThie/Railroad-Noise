...
# whisper brakes
whisbrak <- read.xlsx(file.path(dataPath, "db_bremsen/db_fluesterbremsen.xlsx"), sheetName = "Tabelle1")
...
read_noise <- function(measuring_time, data_folder, noise_type, length_identifier){
  setwd(paste0("N:/Schienenlaerm/data/Laermmessungen/stations/", measuring_time, "/", data_folder))
  file.list <- list.files(path = paste0("N:/Schienenlaerm/data/Laermmessungen/stations/", measuring_time, "/", data_folder), pattern = "*.xlsx", full.names = TRUE)
...
##### apply function
noise_lden <- read_noise(measuring_time = "24h", data_folder = "uptoMar22", noise_type = "noise_lden", length_identifier = 61)
noise_day <- read_noise(measuring_time = "day", data_folder = "uptoMar22", noise_type = "noise_day", length_identifier = 61)
noise_night <- read_noise(measuring_time = "night", data_folder = "uptoMar22", noise_type = "noise_night", length_identifier = 63)
...
# rename
colnames(noise_levels) <- c("date", "station", "noise_lden", "num_pass_lden", "num_freight_lden", "noise_day", "num_pass_day", "num_freight_day", 
...
read_passing <- function(data_folder){
  ...
  file.list <- list.files(path = paste0("N:/Schienenlaerm/data/zugvorbeifahrten/", data_folder), pattern = "*.xlsx", full.names = TRUE)
  df.list <- lapply(file.list, readxl::read_excel)
...
# new passings
new_passings_oct_sep <- passing_oct21[!(passing_oct21$Einfahrtszeit %in% passing_sep21$Einfahrtszeit), ]
new_passings_sep_aug <- passing_sep21[!(passing_sep21$Einfahrtszeit %in% passing_jul21$Einfahrtszeit), ]

# merge old and new
train_passings <- as.data.frame(rbind(passing_jul21, new_passings_sep_aug, new_passings_oct_sep))
...
# rename variables
names(train_passings) <- c("station", "train_type", "duration_sec", "length_m", "speed_kmh", "noise_max_db", "noise_tel_db", "date")
...
map_treat_munc
tmap_save(map_treat_munc, file.path(outputPath, "graphs/map_freight_tracks_treated_munic.png"))
...
map_tracks_treated
tmap_save(map_tracks_treated, file.path(outputPath, "graphs/map_freight_tracks_municipalities.png"))
...
map_buffer3km
tmap_save(map_buffer3km, file.path(outputPath, "graphs/map_freight_tracks_buffer3km.png"))
...
group_summary <- train_passings %>% 
  group_by(train_type) %>% 
  summarise(mean_duration = mean(duration_sec, na.rm = TRUE),
...
# export
openxlsx::write.xlsx(group_summary, file.path(outputPath, "descriptives/group_summary_train_passing.xlsx"), row.names = FALSE, sheetName = "results")
openxlsx::write.xlsx(txt, file.path(outputPath, "descriptives/group_summary_train_passing.xlsx"), row.names = FALSE, col.names = FALSE, append = TRUE, sheetName = "info")
...
# add moving average
day_summary <- day_summary %>% group_by(train_type) %>% mutate(mov_avg = rollmean(mean_noise_tel, k = 14, fill = NA))

# plot
##### owntheme
owntheme <- theme(axis.text.x = element_text(size = 25, angle = 45, hjust = 1),
...
noise_train_passing_type <- ggplot(day_summary)+
  ...
  scale_color_manual(name = "Train Type",
                     values = c("freight_train" = "darkorange2",
                                "passenger_train" = "darkblue"),
                     labels = c("freight_train" = "Freight train",
                                "passenger_train" = "Passenger train"))+
  labs(x = "", y = "TEL Noise Level (in dB)")+
  owntheme

noise_train_passing_type
ggsave(plot = noise_train_passing_type, file.path(outputPath, "graphs/noise_level_by_type_trainpassing.png"), height = 10, width = 13)
...
# export
write.xlsx(des_freight_passings, file.path(outputPath, "descriptives/freight_train_passings.xlsx"), row.names = FALSE)
...
##### summary by day
noise_summary <- noise_levels %>% group_by(date) %>% summarise(mean_noise_lden = mean(noise_lden, na.rm = TRUE),
...
##### summary by weekday
noise_summary_weekday <- noise_levels %>% group_by(weekday) %>% summarise(mean_noise_lden = mean(noise_lden, na.rm = TRUE),
                                                                          mean_num_pass_lden = mean(num_pass_lden, na.rm = TRUE),
                                                                          mean_num_freight_lden = mean(num_freight_lden, na.rm = TRUE),
                                                                          # DAY
                                                                          mean_noise_day = mean(noise_day, na.rm = TRUE),
...
##### summary by month
noise_summary_month <- noise_levels %>% group_by(year_mon) %>% summarise(mean_noise_lden = mean(noise_lden, na.rm = TRUE),
                                                                         mean_num_pass_lden = mean(num_pass_lden, na.rm = TRUE),
                                                                         mean_num_freight_lden = mean(num_freight_lden, na.rm = TRUE),
                                                                         # DAY
                                                                         mean_noise_day = mean(noise_day, na.rm = TRUE),
...
# for monthly data
noise_summary_month <- noise_summary_month %>% mutate(mov_avg_lden = rollmean(mean_noise_lden, k = 3, fill = NA),
                                                      mov_avg_day = rollmean(mean_noise_day, k = 3, fill = NA),
                                                      mov_avg_night = rollmean(mean_noise_night, k = 3, fill = NA))

# export
write.fst(noise_levels, file.path(dataPath, "Laermmessungen/stations/noise_levels_prep.fst"))
write.fst(noise_summary, file.path(dataPath, "Laermmessungen/stations/noise_levels_summary_day.fst"))
write.fst(noise_summary_weekday, file.path(dataPath, "Laermmessungen/stations/noise_levels_summary_weekdays.fst"))
write.xlsx(noise_summary_weekday, file.path(dataPath, "Laermmessungen/stations/noise_levels_summary_weekdays.xlsx"), row.names = FALSE)
...
# calculate average
##### based on daily data
noise_summary$after_treat <- ifelse(noise_summary$date >= "2020-12-13",
                                    yes = 1,
                                    no = 0)
...
mean_noise_treat <- noise_summary %>% 
  ...
  summarise(mean_lden = mean(mean_noise_lden),
...
noise_summary_month$after_treat <- ifelse(noise_summary_month$year_mon >= "2020-12",
                                          yes = 1,
                                          no = 0)

# # mean by group (should give the same as above)
# mean_noise_treat_month <- noise_summary_month %>% 
#   group_by(after_treat) %>% 
#   summarise(mean_lden = mean(mean_noise_lden),
#             mean_day = mean(mean_noise_day),
#             mean_night = mean(mean_noise_night)) %>% 
...
# export
write.xlsx(mean_noise_treat, file.path(outputPath, "descriptives/mean_noise_levels_beforeafter.xlsx"), row.names = FALSE)

# plot date
noise_summary_month$plot_date <- as.yearmon(noise_summary_month$year_mon)

# month labels
month_labels <- c("Apr 2019", "Aug 2019", "Dec 2019", "Apr 2020", "Aug 2020", "Dec 2020", "Apr 2021", "Aug 2021", "Dec 2021")
...
level_by_month_lden <- ggplot(noise_summary_month)+
  geom_line(aes(x = plot_date, y = mean_noise_lden, col = "mean_noise_lden", group = 1), size = 1)+
  geom_line(aes(x = plot_date, y = mov_avg_lden, col = "mov_avg_lden", group = 1), size = 1)+
  scale_color_manual(name = "",
                     values = c("mean_noise_lden" = "darkblue",
                                "mov_avg_lden" = "darkorange3"),
                     labels = c("mean_noise_lden" = "Monthly noise level (LDEN)",
                                "mov_avg_lden" = "3-month average"))+
  scale_y_continuous(breaks = seq(67, 74, 1), limits = c(67, 74))+
  labs(x = "", y = "Average noise level (in dB)")+
  scale_x_yearmon(breaks = seq(min(noise_summary_month$plot_date), max(noise_summary_month$plot_date), 0.34),
                  limits = c(2019.250, 2021.917), labels = month_labels)+
  geom_segment(aes(x = ban, xend = ban, y = 67.3, yend = 73), linetype = 3, size = 2)+
  geom_text(aes(x = 2020.917, y = 73.5, label = "Ban of loud freight trains"), size = 8)+
  owntheme

level_by_month_lden
ggsave(plot = level_by_month_lden, file.path(outputPath, "graphs/noise_level_by_month_LDEN.png"), height = 10, width = 13)

##### Day & Night
level_by_month_daynight <- ggplot(noise_summary_month)+
  geom_line(aes(x = plot_date, y = mean_noise_day, linetype = "mean_noise_day", group = 1), size = 1)+
  geom_line(aes(x = plot_date, y = mean_noise_night, linetype = "mean_noise_night", group = 1), size = 1)+
  scale_linetype_manual(name = "",
                        values = c("mean_noise_day" = "solid",
                                   "mean_noise_night" = "twodash"),
                        labels = c("mean_noise_day" = "Noise level (Day)",
                                   "mean_noise_night" = "Noise level (Night)"))+
  scale_y_continuous(breaks = seq(67, 74, 1), limits = c(67, 74))+
  labs(x = "", y = "Average noise level (in dB)")+
  scale_x_yearmon(breaks = seq(min(noise_summary_month$plot_date), max(noise_summary_month$plot_date), 0.34),
                  limits = c(2019.250, max_date), labels = month_labels)+
  geom_segment(aes(x = ban, xend = ban, y = 67.3, yend = 73), linetype = "dotted", size = 2)+
  geom_segment(aes(x = as.numeric(min(plot_date)), xend = ban, y = before_day, yend = before_day), linetype = "solid", size = 1.5)+
  geom_segment(aes(x = as.numeric(min(plot_date)), xend = ban, y = before_night, yend = before_night), linetype = "twodash", size = 1.5)+
  geom_segment(aes(x = ban, xend = max_date, y = after_day, yend = after_day), linetype = "solid", size = 1.5)+
  geom_segment(aes(x = ban, xend = max_date, y = after_night, yend = after_night), linetype = "twodash", size = 1.5)+
  geom_text(aes(x = ban, y = 73.5, label = "Ban of loud freight trains"), size = 8)+
  owntheme

level_by_month_daynight
ggsave(plot = level_by_month_daynight, file.path(outputPath, "graphs/noise_level_by_month_daynight.png"), height = 10, width = 13)
...
num_trains_month <- ggplot(noise_summary_month)+
  geom_line(aes(x = year_mon, y = mean_num_freight_lden, col = "freight", group = 1), size = 1)+
  geom_line(aes(x = year_mon, y = mean_num_pass_lden, col = "passenger", group = 1), size = 1)+
  scale_color_manual(name = "",
                     values = c("freight" = "seagreen",
                                "passenger" = "firebrick2"),
                     labels = c("freight" = "Freight trains",
                                "passenger" = "Passenger trains"))+
  scale_y_continuous(breaks = seq(80, 220, 20), limits = c(65, 220))+
  labs(x = "", y = "Average number of trains")+
  geom_segment(mapping = aes(x = "2020-12", xend = "2020-12", y = 68, yend = 220), size = 1)+
  geom_text(x = "2020-12", y = 222, label = "Ban of loud freight trains", size = 6)+
  owntheme

num_trains_month
ggsave(plot = num_trains_month, file.path(outputPath, "graphs/num_trains_by_month_LDEN.png"), height = 10, width = 13)

##### LDEN Freight trains
num_freight_trains_month <- ggplot(noise_summary_month)+
  geom_line(aes(x = plot_date, y = mean_num_freight_lden, col = "freight", group = 1), size = 2, show.legend = FALSE)+
  scale_color_manual(name = "",
                     values = c("freight" = "black"))+
  scale_y_continuous(breaks = seq(65, 115, 10), limits = c(65, 115))+
  scale_x_yearmon(breaks = seq(min(noise_summary_month$plot_date), max(noise_summary_month$plot_date), 0.34),
                  labels = month_labels)+
  labs(x = "", y = "Average number of trains")+
  geom_segment(aes(x = 2020.917, xend = 2020.917, y = 68, yend = 110), linetype = 3, size = 2)+
  geom_text(aes(x = 2020.917, y = 112, label = "Ban of loud freight trains"), size = 8)+
owntheme

num_freight_trains_month
ggsave(plot = num_freight_trains_month, file.path(outputPath, "graphs/num_freight_trains_by_month_LDEN.png"), height = 10, width = 13)

###### Freight trains at day and night
num_freight_trains_day_night <- ggplot(noise_summary_month)+
  geom_line(aes(x = plot_date, y = mean_num_freight_day, linetype = "mean_num_freight_day", group = 1), size = 1)+
  geom_line(aes(x = plot_date, y = mean_num_freight_night, linetype = "mean_num_freight_night", group = 1), size = 1)+
  scale_linetype_manual(name = "",
                        values = c("mean_num_freight_day" = "solid",
                                   "mean_num_freight_night" = "twodash"),
                        labels = c("mean_num_freight_day" = "Day",
                                   "mean_num_freight_night" = "Night"))+
  scale_y_continuous(breaks = seq(25, 72, 5), limits = c(25, 73))+
  labs(x = "", y = "Number of freight trains")+
  scale_x_yearmon(breaks = seq(min(noise_summary_month$plot_date), max(noise_summary_month$plot_date), 0.34),
                  limits = c(2019.250, max_date), labels = month_labels)+
  geom_segment(aes(x = ban, xend = ban, y = 25, yend = 72), linetype = "dotted", size = 2)+
  geom_text(aes(x = ban, y = 73, label = "Ban of loud freight trains"), size = 8)+
  geom_smooth(method = "lm", formula = y ~ x, se = FALSE, aes(group = after_treat, x = plot_date, y = mean_num_freight_day), col = "grey60")+
  geom_smooth(method = "lm", formula = y ~ x, se = FALSE, aes(group = after_treat, x = plot_date, y = mean_num_freight_night), col = "grey60")+
  owntheme

num_freight_trains_day_night
ggsave(plot = num_freight_trains_day_night, file.path(outputPath, "graphs/num_freight_trains_day_night.png"), height = 10, width = 13)
...
avg_num_freight <- noise_summary_month |> 
  group_by(after_treat) |> 
  summarise(mean_num_day = mean(mean_num_freight_day, na.rm = TRUE),
            mean_num_night = mean(mean_num_freight_night, na.rm = TRUE)) |> 
  as.data.frame()

write.xlsx(avg_num_freight, file.path(outputPath, "descriptives/avg_num_freight_trains.xlsx"))
...
mean_start <- noise_summary_month %>% 
  ...
  summarise(mean_day = mean(mean_noise_day),
            mean_night = mean(mean_noise_night)) %>% 
  as.data.frame()
...
whisplot <- ggplot(whisbrak, aes(x = jahr))+
  ...
  scale_y_continuous(name = "Number of wagons with whisper brakes (in %)", breaks = seq(7000, 61000, 6000))+
  theme_classic()

whisplot
ggsave(plot = whisplot, file.path(outputPath, "whisper_brakes_db.png"), width = 13, height = 10)
